...
import {
  createDoctorQuery,
  createInsertAvailabilityQuery,
  createInsertSpecialtiesQuery,
  deleteDoctorQuery,
  deleteDoctorSpecialtiesQuery,
  deleteDoctorWeeklyAvailability,
  findAllDoctorsQuery,
  findDoctorByIdQuery,
  findDoctorBySpecialtieQuery,
...
export class DoctorsService {
  ...
  async create(doctor: DoctorDto) {
    ...
    try {
      ...
      const doctorResult = await this.databaseService.query(createDoctorQuery, [
        doctorId,
      ]);

      // Insert specialties into doctor_specialties table
      const insertSpecialtiesQuery = createInsertSpecialtiesQuery(doctor);
      await this.databaseService.query(insertSpecialtiesQuery, [
        doctorId,
        ...doctor.specialties,
      ]);
      ...
      const { insertAvailabilityQuery, valuesArray } =
        createInsertAvailabilityQuery(week_availability, doctorId);
  ...
  async findOne(id: string) {
    try {
      const result = await this.databaseService.query(findDoctorByIdQuery, [
        id,
      ]);
  ...
  async delete(userId: string) {
    try {
      ...
      await this.databaseService.query(deleteDoctorWeeklyAvailability, [
        userId,
      ]);
      ...
      const result = await this.databaseService.query(deleteDoctorQuery, [
        userId,
      ]);
      // End transaction
  ...
  async update(doctor: UpdateDoctorDto) {
    try {
      ...
      await this.databaseService.query(deleteDoctorSpecialtiesQuery, [
        doctor.id,
      ]);
      ...
      if (
        doctor.week_availability &&
        Object.keys(doctor.week_availability).length > 0
      ) {
        const { insertAvailabilityQuery, valuesArray } =
          createInsertAvailabilityQuery(doctor.week_availability, doctor.id);
        await this.databaseService.query(insertAvailabilityQuery, valuesArray);
      }
