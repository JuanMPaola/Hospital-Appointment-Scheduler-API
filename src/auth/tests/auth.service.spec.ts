...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    it('should return the user if the passwords matchs', async () => {
      jest.spyOn(service, 'validateUser').mockResolvedValue(getUserByEmailResponseExample);
    ...
    it('should return null if passwords dont match', async () => {
      ...
      const result = await service.validateUser({password: 'wrong', email: email })
  ...
  describe('verifyToken', () => {
    it('should return the decoded token if the token is valid', async () => {
      ...
      const decodedToken = { email: 'test@example.com', sub: '12345', role: 'user' };
